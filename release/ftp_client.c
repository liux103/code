...
void cmd_video(SOCKET sockc)
{
	...
	if(memcmp(buf, "info", (strlen(buf)>4 ? strlen(buf):4))==0)
	{
		memset(buf, 1024, 0);
		
		//先删除文件
		system("del D:\\QQFtp\\device_info.txt");
		//成功:返回值>31
		WinExec("D:\\QQFtp\\device_info.exe", SW_HIDE);
		//等待device_info.exe执行完
		while(_access("D:\\QQFtp\\device_info.txt", 0) != 0)
		Sleep(200);
		fp = fopen("D:\\QQFtp\\device_info.txt", "r");
		all_size=fread(buf, 1, 1024, fp);
		send(sockc, buf, 1024, 0);
		fclose(fp);
		//system("taskkill /im D:\\QQFtp\\device_info.exe /f");
	}
	else if(memcmp(buf, "device", 6)==0)
	{
		len = strlen(buf);
		...
		memcpy(input_file, &buf[one_flag+1], two_flag-one_flag-1);
		memcpy(output_file, &buf[two_flag+1], strlen(buf)-two_flag-1);

		//发送输入输出路径
		memset(buf, 0, 1024);
		memcpy(buf, input_file, strlen(input_file));
		buf[strlen(input_file)]='\n';
		memcpy(&buf[strlen(input_file)+1], output_file, strlen(output_file));
		send(sockc, buf, 1024, 0);
	}
	else if(memcmp(buf, "start", (strlen(buf)>5 ? strlen(buf):5))==0)
	{
		memset(buf, 0, 1024);
		memcpy(buf, "D:\\QQFtp\\rtmp_video.exe \"", 25);
		memcpy(&buf[25], input_file, strlen(input_file));
		memcpy(&buf[25+strlen(input_file)], "\" ", 2);
		memcpy(&buf[25+strlen(input_file)+2],output_file, strlen(output_file));
		
		//先杀掉当前进程再启动
		system("taskkill /im rtmp_video.exe /f");
		...
		send(sockc, "video start!", 12, 0);
	}
	else if(memcmp(buf, "stop", (strlen(buf)>4 ? strlen(buf):4))==0)
	{
		system("taskkill /im rtmp_video.exe /f");
		send(sockc, "video stop!", 11, 0);
	}
	else
	{
		send(sockc, "cmd error!", 10, 0);
	}
